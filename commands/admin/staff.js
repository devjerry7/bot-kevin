...
module.exports = {
  ...
  async execute(message, args) {
    try {
      // Variáveis seguras para evitar problemas de parser de markdown
      const emojiError = config.emoji?.error || "❌";
      const emojiWarning = config.emoji?.warning || "⚠️";
      const emojiSuccess = config.emoji?.success || "✅";
      const emojiLoading = config.emoji?.loading || "⏳";
      const emojiStats = config.emoji?.stats || "📊";
      ...
      if (
        ...
        return message.reply(
          `${emojiError} Apenas administradores podem usar este comando.`,
        );
      }

      const action = args[0]?.toLowerCase();

      // Comando sem argumentos ou ação inválida mostra o mini-help limpo
      if (
        !["add", "remove", "painel", "perdoar", "testarcron"].includes(action)
      ) {
        return message.reply(
      ...
      if (action === "painel") {
        ...
        if (staffList.length === 0) {
          return message.reply(
            `${emojiWarning} Nenhum membro na equipe no momento.`,
          );
        }
        ...
        const description = staffList
          .map((s) => {
            ...
            const rankConfig =
              staffConfigData.ranks[s.currentRank] || staffConfigData.ranks[1];
            ...
            if (type === "CHAT") {
              ...
              const pChat = Math.min(
                100,
                Math.floor((tr.msgCount / metaC) * 100),
              );
              const pCall = Math.min(
                100,
                Math.floor((tr.voiceMinutes / metaV) * 100),
              );
        ...
        const embed = new EmbedBuilder()
          .setTitle(`${emojiStats} Painel Geral da Equipe`)
      ...
      if (action === "testarcron") {
        ...
        if (staffList.length === 0) {
          return message.reply(
            `${emojiWarning} Nenhum membro na equipe para testar.`,
          );
        }
        ...
        for (const staff of staffList) {
          ...
          const rankConfig =
            staffConfigData.ranks[staff.currentRank] ||
            staffConfigData.ranks[1];
          const type = staff.trackType;

          let bateuMeta = false;
          if (type === "CHAT")
            bateuMeta = tr.msgCount >= (rankConfig.metaChat || 250);
          else if (type === "CALL")
            bateuMeta = tr.voiceMinutes >= (rankConfig.metaCall || 600);
          else
            bateuMeta =
              tr.msgCount >= (rankConfig.metaChat || 250) &&
              tr.voiceMinutes >= (rankConfig.metaCall || 600);

          if (bateuMeta) {
            const novaStreak = staff.streakWeeks + 1;
            relatorio += `${emojiSuccess} <@${staff.discordId}> (${rankConfig.name} - ${type}) - **Meta Batida!** (Streak:${novaStreak})\n`;
          } else {
            relatorio += `${emojiWarning} <@${staff.discordId}> (${rankConfig.name} -${type}) - **Meta Não Batida** (Warning aplicado)\n`;
          }
        }

        await prisma.staffTracking.updateMany({
          data: { msgCount: 0, voiceMinutes: 0 },
        });

        const embed = new EmbedBuilder()
          .setTitle(`${emojiStats} Simulação de Virada de Semana`)
          .setDescription(
            relatorio +
      ...
      if (!targetUser) {
        return message.reply(
          `${emojiError} Você precisa mencionar um membro válido! Ex: \`mc!staff add @usuario chat\``,
        );
      }
      ...
      if (action === "add") {
        ...
        if (!["CHAT", "CALL", "BOTH"].includes(rawType)) {
          return message.reply(
            `${emojiError} Especifique a trilha corretamente: \`chat\`, \`call\` ou \`ambos\`. Ex: \`mc!staff add @user chat\``,
          );
        }
          ...
          if (rolesToAdd.length > 0) {
            await member.roles.add(rolesToAdd).catch((err) => {
              ...
              message.channel.send(
                `${emojiWarning} Membro salvo no banco, mas **falhou ao aplicar o cargo**.`,
              );
            });
          }
        }
        ...
        await targetUser
          .send(
            `${emojiSuccess} Olá, **${targetUser.username}**! Você foi escalado para a equipe de movimentação (**${rawType}**).\n\n` +
              `Suas metas semanais já estão ativas. Acompanhe seu progresso digitando \`mc!meta\` no servidor.`,
          )
          .catch(() => {});

        return message.reply(
          `${emojiSuccess} O usuário <@${targetUser.id}> foi adicionado com sucesso na trilha **${rawType}** (Trainee)!`,
        );
      }
      ...
      if (action === "remove") {
        ...
        return message.reply(
          `${emojiSuccess} O usuário <@${targetUser.id}> foi removido da equipe, teve os cargos retirados e o histórico limpo.`,
        );
      }
      ...
      if (action === "perdoar") {
        ...
        if (!staff) {
          return message.reply(
            `${emojiError} Este usuário não faz parte da equipe.`,
          );
        }

        const rankConfig =
          staffConfigData.ranks[staff.currentRank] || staffConfigData.ranks[1];
        ...
        return message.reply(
          `${emojiSuccess} O progresso do usuário <@${targetUser.id}> foi concluído em 100% para esta semana.`,
        );
      }
    } catch (err) {
      console.error("[ERRO CRÍTICO COMANDO /STAFF]", err);
      return message.reply(
        `${emojiError} Ocorreu um erro interno ao executar este comando.`,
      );
    }
  },
};
